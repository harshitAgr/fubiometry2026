...
def decode_dsnt(logits, temperature=1.0):
    """Full-heatmap DSNT/soft-argmax on pre-sigmoid logits."""
    z = np.asarray(logits, dtype=np.float64) / float(temperature)
    B, K, H, W = z.shape
    z = z - np.max(z, axis=(2, 3), keepdims=True)
    weights = np.exp(z)
    weights /= np.maximum(weights.sum(axis=(2, 3), keepdims=True), 1e-12)
    xs = np.arange(W, dtype=np.float64)[None, None, None, :]
    ys = np.arange(H, dtype=np.float64)[None, None, :, None]
    x = (weights * xs).sum(axis=(2, 3)) / max(W - 1, 1)
    y = (weights * ys).sum(axis=(2, 3)) / max(H - 1, 1)
    return np.stack((x, y), axis=2).reshape(B, 2 * K)
